...
const DAILYMOTION_REGEX =
  /^(https?:\/\/)?(www\.)?dailymotion\.com\/(video|embed\/video)\/([a-zA-Z0-9_-]+)/;
...
const VideoBlock = (
  ...
  if (isEmpty(_url))
    return (
      <EmptySlot blockProps={blockProps} text="VIDEO URL" className="h-36" />
    );
...
registerChaiBlock(VideoBlock as React.FC<any>, {
  ...
  props: {
    ...
    _url: SingleLineText({
      title: "Video URL",
      default:
        "https://www.youtube.com/watch?v=9xwazD5SyVg&ab_channel=MaximilianMustermann",
    }),
    _controls: Model({
      title: "Controls",
      properties: {
